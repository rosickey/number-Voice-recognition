...
*/
(function(){
 var site_list = window.white_site_list || /http:\/\/movie\.douban\.com\/sogou_search|http:\/\/audit\.douban\.com|web2\.qq\.com/;
...
Douban.errdetail = [
    ...
    '淇℃伅涓嶅叏','鍩熷悕閿欒','鍒嗙被閿欒',
    '鐢ㄦ埛閿欒','鏉冮檺涓嶈冻','娌℃湁鏂囦欢',
    '淇濆瓨鏂囦欢閿欒','涓嶆敮鎸佺殑鏂囦欢鏍煎紡','瓒呮椂',
    '鏂囦欢鏍煎紡鏈夎','','娣诲姞鏂囦欢鍑洪敊',
    '宸茬粡杈惧埌瀹归噺涓婇檺','涓嶅瓨鍦ㄧ殑鐩稿唽','鍒犻櫎澶辫触',
...
Douban.init_evc = function(o) {
    ...
    event_monitor.subscribe("entry_"+eid+"_voted", function(caller, data) {
        ...
        if (count) {
            $(o).text(""+count+"浜烘帹鑽�").removeClass("hidden");
        }
    });
}
    ...
    $.postJSON_withck(url, {}, function(ret){
        if (ret.result) {
            if(isnew){
                ...
                $('#voteuse_'+id).html('<span class="m gtleft">浣犵殑鎶曠エ宸茬粡鎻愪氦锛岃阿璋€€�</span>');
                $('#userate_'+id).html('<p id="userate_%s" class="pl">' + ret.usecount + '/' + ret.totalcount + '鐨勪汉瑙夊緱姝よ瘎璁烘湁鐢�:</p>');
            }
        }
        return false;
    });
...
Douban.init_bef = function(o) {
    ...
    unfolder.click(function(){
        if(f.text() == ""){
            var loadtip = $('<div class="loadtip">姝ｅ湪杞藉叆...</div>');
...
Douban.init_closelink = function(o) {
    $('<a href="#">鍏抽棴</a>').appendTo($(o)).click(function() {
        window.close();
        return false;
    });
};
...
Douban.init_interest_form = function(form) {
    ...
    $(form).submit(function() {
        ...
        $('#submits').hide().after('<span class="m" id="saving">姝ｅ湪淇濆瓨...</span>');
        refine_dialog();
        return false;
    });
...
Douban.init_stars = function(o){
    var ratewords = {1:'寰堝樊', 2:'杈冨樊', 3:'杩樿', 4:'鎺ㄨ崘', 5:'鍔涜崘'},
...
var show_dialog = function(div, w) {
        ...
        if($('#hiddendialog').length){
            ...
            $.getJSON(url, function(r) {
                if($('#dialog').length){
                    ...
                    populate_tag_btns('鎴戠殑鏍囩:', $('#mytags', html), r['my_tags'], hash);
                    populate_tag_btns("甯哥敤鏍囩:", $('#populartags', html), r['popular_tags'], hash);
                    ...
                    if(btn_type == 'cbtn'){
                        var h2 = $('h2','#dialog');
                        h2.text(h2.text().replace('淇敼','鍐欑煭璇�'));
...
Douban.init_nine_collect_btn = function(o) {
    $(o).click(function(){
        ...
        $.getJSON(url, interest && {interest: interest}, function(r) {
            ...
            var hash = {};
            $.each(tags, function(i,tag){hash[tag.toLowerCase()]=true;});
            populate_tag_btns('鎴戠殑鏍囩(鐐瑰嚮娣诲姞):', $('#mytags', html), r.my_tags, hash);
            populate_tag_btns("璞嗙摚鎴愬憳甯哥敤鐨勬爣绛�(鐐瑰嚮娣诲姞):", $('#populartags', html), r.popular_tags, hash);
            if (btn_type == 'pbtn')
                $('form', html).data('reload',1);
...
$(".recsubmit").before('<div class="attn" style="text-align:center" id="ban_word">浣犵殑鎺ㄨ崘涓湁琚姝㈢殑鍐呭</div >')
                return
            }
            $('#dialog').html('<div class="loadpop m">鎺ㄨ崘宸叉彁浜�</div>');
            set_overlay();
...
Douban.init_reply_form = function(form){
    ...
    $(form).submit(function(){
        ...
        if (n=='n'){
        var a = $('<span><a href="javascript:void(0)">娣诲姞鍥炲簲</a></span>');
        }
        else
        {
            var a = $('<span style="margin-left:53px"><a href="javascript:void(0)">娣诲姞鍥炲簲</a></span>');
        }
...
Douban.init_editable = function(o){
    ...
    var show = function(t){
        if(t != undefined){
            disp.text(t);
            if (disp.text() == ''){
                a.text('鐐瑰嚮娣诲姞鎻忚堪').addClass('sign-text');
            }else{
                a.text('淇敼').removeClass('sign-text');
            }
        }
        disp.show();
        $(form).hide();
    ...
    set_len_limit : function(limit){
        ...
        var check = function(){
            if(this.value && this.value.length > limit){
                s.attr('disabled',1).attr('value','瀛楁暟涓嶈兘瓒呰繃'+limit+'瀛�');
            } else {
...
Douban.init_song_interest = function(o) {
    ...
    $(o).click(function() {
        ...
        $.post_withck(url, {'action':(i?'n':'y')},function(ret) {
            ...
            if(i) {
                $(o).children().attr({src:"/pics/gray-heart.gif",title:"鎴戝枩娆�", alt:"鎴戝枩娆�"});
            } else {
                $(o).children().attr({src: "/pics/red-heart.gif",title:"鍙栨秷'鎴戝枩娆�'", alt:"鍙栨秷'鎴戝枩娆�'"});
            }
        }, 'text');
        return false;
    });
...
function cinema_full() {
    $('#info_area').html('姝ｅ湪鏌ヨ锛岃绋嶇瓑...');
    $('#info_area').load_withck("/j/movie/cinemainfo",{id:movie_id, city:at_city, day:delta_days, view:view})
}
...
Douban.init_nointerest_subject_movie = function(o){
    $(o).click(function(){
        ...
        //    is_blacklist = true;
        //}else{
        //    is_blacklist = confirm("浣犵‘瀹氫笉鍐嶅叧娉ㄨ繖閮ㄧ數褰�?");
        //}
        _self = this;
        if (is_blacklist){
            tag = $(".tag-fav-cloud > .current").attr("name");
...
Douban.init_donate = function(){
  var templ_container  = '<div class="blocktip dou-tip">{BODY}</div>',
    templ_form = '<form action="" method="post"><div class="frm-item">浣犲皢鍚戜綔鑰呰禒閫�<b>1</b>棰楀皬璞�</div><div class="frm-item"><label for="dou-inp-msg">椤哄甫鎹庝釜璇�...</label><input id="dou-inp-msg" type="text" name="note"></div><div class="frm-submit"><span class="bn-flat"><input type="submit" value="閫佸嚭"></span><a href="#" class="tip-bn-cancel">鍙栨秷</a></div></form>',
    templ_tip = '<p>鈥滄劅璋⑩€濆皢鍚戜綔鑰呰禒閫�<b>1</b>棰楀皬璞嗭紝浣犺繕娌℃湁灏忚眴銆�<br><a href="http://www.douban.com/help/account#t4-q1">鎬庢牱鑾峰彇灏忚眴?</a></p><span class="bn-flat"><input type="button" class="tip-bn-cancel"  value="鐭ラ亾浜�"></span>',
    templ_error = '<span class="donated-fail">{MSG}</span>',
    templ_success = '<span class="donated-success">{MSG}</span>',
    templ_processing = '<p>澶勭悊涓紝璇风◢鍊�...</p>',
...
var Muggy={};Muggy.Utils={};Muggy.BAND_TYPE="B";Muggy.SINGLE_TYPE="S";Muggy.recentStart=0;Muggy.popularStart=0;Muggy.friendStart=0;Muggy.HEARTTYPE="1";Muggy.SKIPTYPE="0";
Muggy.initShare=function(){$("body").delegate(".share","mouseenter",function(){$(this).find(".share-sites").fadeIn();var b=$(this).attr("id");Muggy.share($(this).find(".share-sites"),b);$(this).parent().find(".option-wrapper").hide()});$("body").delegate(".share","mouseleave",function(){$(this).find(".share-sites").hide();$(this).find(".share-sites").undelegate("a","click");$(this).parent().find(".option-wrapper").show()})};
Muggy.initPageCardHover=function(){$("body").delegate(".gene-content","mouseenter",function(){$(this).parent(".gene").find(".listen-to-card").show();$(this).find("ol").addClass("blur")});$("body").delegate(".gene-content","mouseleave",function(){$(this).parent(".gene").find(".listen-to-card").hide();$(this).find("ol").removeClass("blur")})};
Muggy.initSlideGeneCard=function(){var b=$(".btn-next"),c=$(".btn-prev");b.click(function(){if($(this).hasClass("recent"))Muggy.loadRecentGeneList();else if($(this).hasClass("popular"))Muggy.loadPopularGeneList();else if($(this).hasClass("friend")){var a=$("#user-id-holder").val();Muggy.loadFriendGeneList(a)}a=$(this).next().next().find(".current-gene-list");a.animate({left:"-874px"},"slow");a.removeClass("current-gene-list");a.next().animate({left:"0px"},"slow");a.next().addClass("current-gene-list")});
c.click(function(){var a=$(this).next().find(".current-gene-list");a.hasClass("first-gene-list")||(a.animate({left:"874px"},"slow"),a.removeClass("current-gene-list"),a.prev().animate({left:"0px"},"slow"),a.prev().addClass("current-gene-list"))})};Muggy.artistIdList=[];Muggy.skipIdList=[];
Muggy.initBtnAction=function(){$("#btn-start").click(function(){window.location="/testing"});$(document).delegate(".gene-collect","click",function(){var b=$(this),c=b.closest("li").attr("id").split("-")[1];$.post_withck("/api/save_collection",{gene_id:c},function(a){a=$.parseJSON(a);if(a==="N")window.location="http://douban.com/accounts/login";else if(a==="X")Muggy.Utils.popDialog("collection-full-dialog");else if(a==="D")Muggy.Utils.popDialog("collect-duplicate-dialog");else{if($("#this-is-mine-page").length>
0){var d=b.closest("li").clone();d.find(".option-wrapper").html('<a class="gray-font drop-collected" href="javascript:;" data-gid="'+c+'">\u4e22\u5f03</a> \u00b7 <a href="http://douban.fm/?context=channel:0|muggy:'+a+'" target="_blank">\u6536\u542c</a>');$($(".gene-to-collect").closest("li")[0]).html(d.html())}b.replaceWith('<a href="javascript:;" class="gene-uncollect">\u53d6\u6d88\u6536\u85cf</a>')}})});$(document).delegate(".gene-uncollect","click",function(){var b=$(this),c=b.closest("li").attr("id").split("-")[1],
a=$("#my-collection-gene-"+c);$.post_withck("/api/drop_collected_gene",{gid:c},function(d){d=$.parseJSON(d);b.replaceWith('<a href="javascript:;" class="gene-collect">\u6536\u85cf</a>');a.find(".gene").fadeOut("slow",function(){a.html('<div class="gene-holder"><div class="gene-to-collect"></div></div>')})})});$(".add-gene").click(function(){window.location="/testing"})};
Muggy.initTestingPageBtnAction=function(){Muggy.Utils.checkGenePort();$(".op-left, .op-right");var b=$("#change-artists"),c=$(".tester-progress-wrapper"),a=$(".tester-progress");$("#progress-prompt");var d=c.width()/12;a.width(d);$("body").delegate(".op-left, .tester-left, .left-arrow","mouseenter",function(){$(".left-arrow").addClass("show-left-arrow")});$("body").delegate(".op-right, .tester-right, .right-arrow","mouseenter",function(){$(".right-arrow").addClass("show-right-arrow")});$("body").delegate(".op-left, .tester-left, .left-arrow",
"mouseleave",function(){$(".left-arrow").removeClass("show-left-arrow")});$("body").delegate(".op-right, .tester-right, .right-arrow","mouseleave",function(){$(".right-arrow").removeClass("show-right-arrow")});$("body").delegate(".op-left, .op-right, .tester-left, .tester-right","click",function(){var b=$(this);b=(b.hasClass("op-left")?$(".tester-left"):b.hasClass("op-right")?$(".tester-right"):b).find(".artist_id").val();Muggy.artistIdList.push(b);Muggy.showLoadingImage();Muggy.sendArtist(Muggy.HEARTTYPE);
a.width()<c.width()&&a.animate({width:a.width()+d})});b.click(function(){Muggy.showLoadingImage();var a=$(".tester-left").find(".artist_id").val(),b=$(".tester-right").find(".artist_id").val();Muggy.skipIdList.push(a);Muggy.skipIdList.push(b);Muggy.sendArtist(Muggy.SKIPTYPE)});$("body").delegate("#save-result","click",function(){var a=$("#gene-id").val(),b=$("#ref-gene-id").val();$.post_withck("/api/save_card",{gene_id:a,ref_gene_id:b},function(a){a=$.parseJSON(a);a==="X"?Muggy.Utils.popDialog("gene-full-dialog"):
window.location="/mine#mine-generated-gene"})})};Muggy.showLoadingImage=function(){$("div.artist-avatar").html('<img class="loading-artist-img" src="http://img3.douban.com/anduin/pics/loading.gif?v=2" width="26px" height="8px" alt="\u8bfb\u53d6\u4e2d..."/>')};
Muggy.initMinePageBtnAction=function(){$(".drop-collected");$(document).delegate(".drop-collected","click",function(){var b=$(this).closest(".gene-card");$.post_withck("/api/drop_collected_gene",{gid:$(this).attr("data-gid")},function(){b.find(".gene").fadeOut("slow",function(){b.html('<div class="gene-holder"><div class="gene-to-collect"></div></div>')})})});$("#share-my-card").click(function(){})};
Muggy.sendArtist=function(b){$.post_withck("/api/artists_pair",{id_list:Muggy.artistIdList,skip_list:Muggy.skipIdList,op_type:b},function(b){b=$.parseJSON(b);console.log(b);if(typeof b==="string")$("#data-holder").append('<input type="hide" value="'+b+'" id="gene-id"/>'),Muggy.endTesting(b);else{var a=$("#tester-template").html(),d=b[0];b=b[1];$(".tester-right").html($.tmpl(a,{artist_id:b.id,direction:b.type===Muggy.BAND_TYPE?"artist-avatar-horizon":"artist-avatar-vertical",img_url:b.img_url,position:"right",
artist_name:b.name}));$(".tester-left").html($.tmpl(a,{artist_id:d.id,direction:d.type===Muggy.BAND_TYPE?"artist-avatar-horizon":"artist-avatar-vertical",img_url:d.img_url,position:"left",artist_name:d.name}))}})};Muggy.endTesting=function(b){var c=$("#gene-generating").html(),a=$("#testing-result").html();$(".bd").html($.tmpl(c,{}));$.post_withck("/api/get_result_card",{gene_id:b},function(b){b=$.parseJSON(b);$(".bd").html($.tmpl(a,{musicians:b}))})};
Muggy.loadRecentGeneList=function(){var b=$("#gene-list-template").html(),c=$(".recent-gene-list");Muggy.recentStart===0?$.post_withck("/api/recent_gene",{s:Muggy.recentStart},function(a){a=$.parseJSON(a);a=$.tmpl(b,{cards:a});c.append(a).find("ul").removeClass("l874").addClass("current-gene-list").addClass("first-gene-list");Muggy.recentStart+=4;$.post_withck("/api/recent_gene",{s:Muggy.recentStart},function(a){a=$.parseJSON(a);a=$.tmpl(b,{cards:a});c.append(a);Muggy.recentStart+=4})}):$.post_withck("/api/recent_gene",
{s:Muggy.recentStart},function(a){a=$.parseJSON(a);a=$.tmpl(b,{cards:a});c.append(a);Muggy.recentStart+=4})};
Muggy.loadPopularGeneList=function(){var b=$("#gene-list-template").html(),c=$(".popular-gene-list");Muggy.recentStart===0?$.post_withck("/api/popular_gene",{s:Muggy.popularStart},function(a){a=$.parseJSON(a);a=$.tmpl(b,{cards:a});c.append(a).find("ul").removeClass("l874").addClass("current-gene-list").addClass("first-gene-list");Muggy.popularStart+=4;$.post_withck("/api/popular_gene",{s:Muggy.popularStart},function(a){a=$.parseJSON(a);a=$.tmpl(b,{cards:a});c.append(a);Muggy.popularStart+=4})}):$.post_withck("/api/popular_gene",
{s:Muggy.popularStart},function(a){a=$.parseJSON(a);a=$.tmpl(b,{cards:a});c.append(a);Muggy.popularStart+=4})};
Muggy.loadFriendGeneList=function(b){var c=$("#gene-list-template").html(),a=$(".friend-gene-list");Muggy.friendStart===0?$.post("/api/friend_gene",{s:Muggy.friendStart,user_id:b},function(d){d=$.parseJSON(d);d=$.tmpl(c,{cards:d});a.append(d).find("ul").removeClass("l874").addClass("current-gene-list").addClass("first-gene-list");Muggy.friendStart+=4;$.post_withck("/api/friend_gene",{s:Muggy.friendStart,user_id:b},function(b){b=$.parseJSON(b);b=$.tmpl(c,{cards:b});a.append(b);Muggy.friendStart+=4})}):
$.post_withck("/api/friend_gene",{s:Muggy.friendStart,user_id:b},function(b){b=$.parseJSON(b);b=$.tmpl(c,{cards:b});a.append(b);Muggy.friendStart+=4})};
Muggy.Utils.popDialog=function(b){$("#wrapper").addClass("overlay");$("#"+b).dialog({height:250,width:500,close:function(){$("#wrapper").removeClass("overlay");if(RegExp(/\/testing/).test(document.URL))window.location="/mine"}});$("#gene-full-dialog .fleft, #collection-full-dialog .fleft").click(function(){window.location="doublo.labs.douban.com"});$("#gene-full-dialog .fright, #collection-full-dialog .fright").click(function(){window.location="/mine"})};
Muggy.Utils.checkGenePort=function(){$("#ref-gene-id").val()===""&&$.post_withck("/api/has_gene_port",function(b){b==="N"&&Muggy.Utils.popDialog("gene-full-dialog")})};
(function(){var h=window.Muggy||{},g=window.screen,n=window.navigator,o=window.setTimeout,p="http://www.douban.com/link2?type=redir&vendor=bshare_sina&url=",j=decodeURI,i=encodeURI,k={douban:{method:function(a,e,d,b){var c="",f="";b===""?(c="http://muggy.labs.douban.com/"+a,f=e+"\u7684\u97f3\u4e50\u57fa\u56e0\u540d\u7247"):(c=b,f="\u6536\u542c"+e+"\u5728\u8c46\u74e3\u5b9e\u9a8c\u5ba4\u751f\u6210\u7684\u97f3\u4e50\u57fa\u56e0\u7247\u6bb5");a="http://shuo.douban.com/!service/share?"+$.param({name:f,
href:c,desc:"\u6765\u81eaMUGGY@DoubanLabs",text:"#Muggy#",image:d,apikey:"079055d6a0d5ddf816b10183e28199e8"});(a=window.open(a,"",["toolbar=0,status=0,resizable=1,width=500,height=360,left=",(g.width-500)/2,",top=",(g.height-360)/2].join("")))&&a.focus()}},sina:{method:function(a,e,d,b){var c="",f="";b===""?(c="http://muggy.labs.douban.com/"+a,f="#Muggy#"+e+"\u7684\u97f3\u4e50\u57fa\u56e0\u540d\u7247"):(c=b,f="#Muggy#\u6536\u542c"+e+"\u5728\u8c46\u74e3\u5b9e\u9a8c\u5ba4\u751f\u6210\u7684\u97f3\u4e50\u57fa\u56e0");
(function(a,d,b,c,e,f,g,h,j,i){function l(){var d=p+b([k,f].join(""));if(!window.open(d,"mb",["toolbar=0,status=0,resizable=1,width=440,height=430,left=",(a.width-440)/2,",top=",(a.height-430)/2].join("")))m.href=d}var k="http://v.t.sina.com.cn/share/share.php?appkey=3163308509",m=h||d.location;f=["&url=",b(m),"&title=",b(g||d.title),"&source=",b(c),"&sourceUrl=",b(e),"&content=",i||"gb2312","&pic=",b(f||"")].join("");/Firefox/.test(n.userAgent)?o(l,0):l()})(g,document,encodeURIComponent,"","",d,
j(f),j(c),"","utf-8")}},msn:{label:"\u5206\u4eab\u5230MSN",method:function(a,e,d,b){var c="",f="";b===""?(c="http://muggy.labs.douban.com/"+a,f=e+"\u7684\u97f3\u4e50\u57fa\u56e0\u540d\u7247"):(c=b,f="\u6536\u542c"+e+"\u5728\u8c46\u74e3\u5b9e\u9a8c\u5ba4\u751f\u6210\u7684\u97f3\u4e50\u57fa\u56e0\u7247\u6bb5");a="http://www.douban.com/link2?type=redir&vendor=bshare_msn&url="+i("http://profile.live.com/badge?url="+encodeURIComponent(c)+"&title="+encodeURIComponent(f)+"&screenshot="+encodeURIComponent(d));
(a=window.open(a,"msn",["toolbar=0,status=0,resizable=1,width=600,height=500,","left="+(g.width-600)/2,",top="+(g.height-500)/2].join("")))&&a.focus()}},kx:{label:"\u5206\u4eab\u5230\u5f00\u5fc3\u7f51",method:function(a,e,d,b){var c=d="";b===""?(d="http://muggy.labs.douban.com/"+a,c=e+"\u7684\u97f3\u4e50\u57fa\u56e0\u540d\u7247"):(d=b,c="\u6536\u542c"+e+"\u5728\u8c46\u74e3\u5b9e\u9a8c\u5ba4\u751f\u6210\u7684\u97f3\u4e50\u57fa\u56e0\u7247\u6bb5");a="http://www.douban.com/link2?type=redir&vendor=bshare_kx&url="+
encodeURIComponent("http://www.kaixin001.com/repaste/bshare.php?rurl="+encodeURIComponent(d)+"&rcontent=&rtitle="+c);(a=window.open(a,"kx",["toolbar=0,status=0,resizable=1,width=600,height=360,","left="+(g.width-600)/2,",top="+(g.height-360)/2].join("")))&&a.focus()}},renren:{method:function(a,e,d,b){var c=d="";b===""?(d="http://muggy.labs.douban.com/"+a,c=e+"\u7684\u97f3\u4e50\u57fa\u56e0\u540d\u7247"):(d=b,c="\u6536\u542c"+e+"\u5728\u8c46\u74e3\u5b9e\u9a8c\u5ba4\u751f\u6210\u7684\u97f3\u4e50\u57fa\u56e0\u7247\u6bb5");
a="http://www.douban.com/link2?type=redir&vendor=bshare_renren&url="+i("http://www.connect.renren.com/share/sharer?title="+encodeURIComponent(c)+"&url="+encodeURIComponent(d));(a=window.open(a,"rr",["toolbar=0,status=0,resizable=1,width=510,height=300,","left="+(g.width-510)/2,",top="+(g.height-300)/2].join("")))&&a.focus()}}};h.getShareObject=function(a){return k[a]};h.share=function(a,e){a.delegate("a","click",function(a){a.preventDefault();var b=$(a.currentTarget);if(e==="share-my-card")a=window.document.tagcloud.getScreenshot(),
$.ajax_withck({type:"POST",url:"/api/screenshot",data:{imagedata:a},async:!1,success:function(a){var c=b.attr("class").replace("lnk-",""),d=$("#this-is-mine-page").val();(c=h.getShareObject(c))&&c.method(d,"\u6211",a,"")}});else{a=b.closest(".gene-card").find("#gid").val();var c=b.attr("class").replace("lnk-",""),f=b.closest(".gene-card").find(".card-owner-id").val(),g=b.closest(".gene-card").find(".listen-to-card a").attr("href"),i=b.closest(".gene").find(".user-info-name").html();$.ajax_withck({type:"POST",
url:"/api/gene_rec",data:{gene_id:a},async:!1,success:function(a){window.open();var b=h.getShareObject(c);b&&b.method(f,i.trim(),a,g)}})}})};window.Muggy=h})();
